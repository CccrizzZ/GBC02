...
void StartScene::draw()
{
	...
	if (m_displayUI)
	{
		...
		for (int i = 0; i < m_Bullet.size(); i++)
		{
			Util::DrawRect(m_Bullet[i]->getPosition() - glm::vec2(m_Bullet[i]->getWidth() * 0.5f, m_Bullet[i]->getHeight() *0.5f), m_Bullet[i]->getWidth(), m_Bullet[i]->getHeight());
...
void StartScene::update()
{
	...
	//CollisionManager::squaredRadiusCheck(m_pShip, m_Bullet);


		for (int i = 0; i < m_Bullet.size(); i++)
		{
			m_Bullet[i]->update();
			CollisionManager::AABBCheck(m_pShip, m_Bullet[i]);
		}
...
void StartScene::start()
{
	...
	for (int i = 0; i < 9; i++)
	{
		...
		temp->setPosition(glm::vec2(randposX, randposY));
